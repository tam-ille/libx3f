...
/*   unsigned *oprof; */
  ...
  tmp=(int16_t (*)[4])interpolated->img;
...
void simple_coeff (INTERPOLATED_IMG *img, int index)
{
  ...
  { 1.1032,-0.5231,-0.2016,-0.7263,1.1816,0.017,-0.0112,0.0183,1.0113 },
  };
  int i, c;
...
void foveon_f20_interpolate(INTERPOLATED_IMG *i_img, X3F *x3f)
{
  ...
  float fsum[3], val, frow, num, pgain[4];;
  ...
  foveon_fixed (camf, ppm, 9, "CP2_Matrix");
  ...
  /* try to first convert raw datas to XYZ using ColorModeCompensation */
  /* then do the interpolation */
  /* then convert to RGB using WhiteBalanceColorCorrections and WhiteBalanceGains */
